...
bool grug_init(grug_runtime_error_handler_t handler, const char *mod_api_json_path, const char *mods_dir_path, uint64_t on_fn_time_limit_ms, struct grug_backend *backend) __attribute__((warn_unused_result));
...
// - From https://eklitzke.org/path-max-is-tricky
...
struct grug_file {
	...
	// TODO: Should the function pointers be offered by the backend implementation,
	// since for example an interpreter may not allow game devs to call on_ fn pointers directly?
	void *on_fns;
...
// These allow game functions to print more runtime error details.
